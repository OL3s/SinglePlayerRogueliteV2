...
public partial class TouchStick : Panel
{
	...
	private void HandleScreenTouch(InputEventScreenTouch touchEvent)
	{
		...
		{
			if (_activeFingerIndex != -1 || !GetGlobalRect().HasPoint(touchEvent.Position))
			{
				return;
			}
		...
		if (touchEvent.Index != _activeFingerIndex)
		{
			return;
		}
	...
	{
		if (dragEvent.Index != _activeFingerIndex)
		{
			return;
		}
	...
	private void UpdateTapCandidate(Vector2 globalPosition)
	{
		...
		if (!IsWithinDeadzone(globalPosition))
		{
			_tapCandidate = false;
		}
	}
	...
	{
		if (maxDistance <= 0.0f || clampedLength <= 0.0f)
		{
			return Vector2.Zero;
		}
		...
		if (normalizedMagnitude <= clampedDeadzone)
		{
			return Vector2.Zero;
		}
